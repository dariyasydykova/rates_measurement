...
#include "LG_aa.mdl"; // Can't find a way to have this file name be a variable in setup. Unfortunate. Either way, it currently also contains a frequency vector (equal) called JC_freqs.
...
Model LGfull = (LG_t, obsFreqsFull, 1);
...
for (global site_count = 0; site_count < nsites; site_count = site_count+site_dupl)
{
    ...
    Model LGsite = (LG_rt, obsFreqsFull, 1);
    UseModel(USE_NO_MODEL);
    UseModel(LGsite);
