...
r_an <- read.table("analytically_derived_rates/rates_all_sites.txt",header=T)
r_inf <- read.csv("inferred_rates/processed_rates/rates_all_sites.csv")

#get the mean inferred rate at each time point
r_an_temp <- r_an %>% 
  group_by(site) %>% 
  summarise(r_tilde_ms_small_t=r_tilde_ms_small_t[1]) 
...
r <- r_inf %>% 
  ...
  mutate(inf_rate_mean=mean(inf_rate),inf_rate_norm=inf_rate/inf_rate_mean)  %>%
  filter(num_taxa==512) %>%
...
p <- ggplot(r,aes(r_tilde_ms_small_t,inf_rate_norm_mean)) +
    #background_grid("xy")+
