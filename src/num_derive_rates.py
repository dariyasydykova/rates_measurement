...
def get_site_sums(total_sites,pi_data,q_data,make_true_r = False,site_specific = True):
	site_sum_dict = {}
	...
	if make_true_r:
		return site_sum_dict,true_r_dict
	else: 
...
def main():
	...
	site_sums_ms = get_site_sums(total_sites, pi_dict_ms, q_dict_ms)
	site_sums_jc,true_r_dict = get_site_sums(total_sites, pi_dict_jc, q_dict_jc, make_true_r=True, site_specific=False)
	...
	print r_tilde_dict_ms_normalized
	out.write("site\ttime\tr_tilde_ms\tr_tilde_ms_norm\tr_tilde_jc\tr_tilde_jc_norm\ttrue_r\n")
	t_lst = np.arange(0.000002,2,0.02)
	for site in ddg_dict:
		for i in range(len(t_lst)):
			line = str(site-1)+ "\t" +str(t_lst[i])+ "\t"+ str(r_tilde_dict_ms[site][i])+"\t"+str(r_tilde_dict_ms_normalized[site][i])+"\t"+str(r_tilde_dict_jc[site][i])+"\t"+str(r_tilde_dict_jc_normalized[site][i])+"\t"+str(true_r_dict[site])+"\n"
			out.write(line)
