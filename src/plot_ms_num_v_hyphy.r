...
for (i in c(1:10)){
  ...
  p_rates <- ggplot() +
    ...
    geom_errorbar(data=r_jc_ef,aes(x=time,y=rate/rate_mean,group=time),color="red", width=.01) +
    geom_boxplot(data=r_jc,aes(x=time,y=rate/rate_mean,group=time),color="purple", width=.01) +
